...
systemParameters.eNBonly = 1; % (=1) only eNB no LiFi; (=0) eNB + LiFi
systemParameters.poissonEventPerMin = 10;
...
% will be (i) dropped if it requested service from eNB; or 
% (ii) handover to eNB if it is requested service from LiFi AP
systemParameters.sessionTimeOut = 1e2; % [ms] 

systemParameters.numBS = 1;
systemParameters.Nt = 4;
systemParameters.numUserPerAP = ceil(50/systemParameters.Nt);

%% Open Threads as required
% parpool(numberThreads);

%% Load previous UE and eNB data
load('previousUEandeNBdata.mat');
...
for i=1:iterations
    ...
    if ~isfield(UElocs,'x')
        ...
        userLocations(:,1) = UElocs.x';
        userLocations(:,2) = UElocs.y';
        userLocations(:,3) = UElocs.z';
        eNBlocations(:,1) = eNBlocs.x';
        eNBlocations(:,2) = eNBlocs.y';
        eNBlocations(:,3) = eNBlocs.z';
        [~,APlocations,~ ] =initGeometry(systemParameters);
        plotGeometry
        clear eNBlocs UElocs
    end
    ...
    eNBdataRate{i} = mat2cell(repmat(zeros(totalUsers,1),1,max(eventTime)),totalUsers,repmat(1,1,max(eventTime)));
    LAPdataRate{i} = mat2cell(repmat(zeros(totalUsers,1),1,max(eventTime)),totalUsers,repmat(1,1,max(eventTime)));
    ...
    UE.LAPsessionDrop = zeros(totalUsers,actualTTI);
    UE.eNBsessionDrop = zeros(totalUsers,actualTTI);
    ...
    for l=1:systemParameters.Nt
        LED.Smetric{l} = -Inf(1,length(LED.connectedUE{l}));
    end
    ...
    while flag < 1
            ...
            for usind=userIndex
                ...
                if LEDindex ~= 0
                    ...
                    if LED.cachedFile(LEDindex,requestedContent) == 1 && systemParameters.eNBonly == 0
                        ...
                        LED.Smetric{LEDindex}(LED.connectedUE{LEDindex}==usind) = Inf;
                    else % activate eNB
                        ...
                        eNB.Smetric{eNBindex}(eNB.connectedUE{eNBindex}==usind) = Inf;
                    end
                else
                    % activate eNB
                    UE.activeBSs(usind) = eNBindex;
        ...
        if size(eNBallocation,2) < actualTTI
            eNBallocation{actualTTI}            = [];
            eNBSINR{actualTTI}                  = [];
            eNBinterferenceOut{actualTTI}       = [];
            eNBSINRTargets{actualTTI}           = [];
            eNBbitsSymbol{actualTTI}            = [];
            eNBtransmissionPower{actualTTI}     = [];
            eNBscheduledUserOut{actualTTI}      = [];
            eNBflaggedCHs{actualTTI}            = [];
            eNBschedulingObjective{actualTTI}   = [];
        end
        if size(LAPallocation,2) < actualTTI
            LAPallocation{actualTTI}            = [];
            LAPSINR{actualTTI}                  = [];
            LAPinterferenceOut{actualTTI}       = [];
            LAPSINRTargets{actualTTI}           = [];
            LAPbitsSymbol{actualTTI}            = [];
            LAPtransmissionPower{actualTTI}     = [];
            LAPscheduledUserOut{actualTTI}      = [];
            LAPflaggedCHs{actualTTI}            = [];
            LAPschedulingObjective{actualTTI}   = [];
        end
        ...
        if isempty([UE.activeUsers_eNB, UE.activeUsers_LAP])
            ...
            if ~isempty(UE.activeUsers_eNB)
                ...
                [UE, eNB, LED] = updateStatus(UE, eNB, LED, actualTTI, eNBdataRate{i}, systemParameters.sessionTimeOut, WAT);
                WAT = [];
            end
            ...
            if ~isempty(UE.activeUsers_LAP)
                ...
                LAPcurrentAllocationIN = zeros(systemParameters.Nt, systemParameters.LAPnumRBs);
                ...
                %% Update status of active users
                [UE, eNB, LED] = updateStatus(UE, eNB, LED, actualTTI, LAPdataRate{i}, systemParameters.sessionTimeOut, WAT);
                WAT = [];
            end
            % Go to the next TTI
            actualTTI = actualTTI + 1;
...
if systemParameters.eNBonly == 0
    save('UEdata_eNBLiFi_4APs','UE','eNB','LED','systemParameters')
else
    save('UEdata_eNB_4APs','UE','eNB','systemParameters')
end
